            ...
            @if (state.phase() === 'idle') {
                ...
                <div class="review-section">
                  <div class="review-header">
                    ...
                    <span class="review-count mono">{{
                      asArr(result['review_queue']).length
                    }}</span>
                  </div>
                  @for (item of asArr(result['review_queue']); track item['step_id']) {
                    <div class="review-item">
                      <div class="review-item-top">
                        <span
                          class="ptype-mini"
                          [class]="'ptype-' + primTypeOf(item['primitive'])"
                          >{{ primTypeLabel(item['primitive']) }}</span
                        >
                        <span class="review-item-prim mono">{{ item['primitive'] }}</span>
                        <span class="review-item-conf conf-low mono"
                      ...
                      <div class="review-item-action">
                        Output included in results — manual verification recommended before use in
                        production.
                      </div>
                    </div>
                  }
              ...
              @if (waterfallSteps(result['answer']).length > 0) {
                <div class="answer-section">
                  <div class="sec-label">
                    PRIORITY OF PAYMENTS — {{ waterfallSteps(result['answer']).length }} STEPS
                  </div>
                ...
                <div class="comparisons-section">
                  ...
                  @for (cmp of asArr(result['comparisons']); track cmp['term']) {
                    <div class="cmp-row" [class.material]="cmp['materiality'] === 'material'">
                      ...
                      <div class="cmp-mat-badge" [class]="'mat-' + cmp['materiality']">
                        {{ cmp['materiality'] }}
                      </div>
              ...
              @if (asArr(result['assessments']).length > 0) {
                <div class="assessments-section">
                  ...
                  @for (a of asArr(result['assessments']); track a['claim']) {
                    <div class="asm-row">
                      <div class="asm-verdict" [class]="'verdict-' + a['verdict']">
                        {{ a['verdict'] }}
                      </div>
                  ...
                  <div class="conf-spark">
                    @for (c of confScores(); track $index) {
                      <div
                        ...
                        [title]="c.primitive + ': ' + c.confidence.toFixed(2)"
                      >
                        <div
                          class="spark-bar"
                          [style.height.%]="c.confidence * 100"
                          [class]="confBarClass(c.confidence)"
                        ></div>
              ...
              @for (step of asArr(traceData()!['steps']); track step['step_id']) {
                <div class="trace-step" [class.expanded]="expandedStep() === step['step_id']">
                  <button class="trace-step-header" (click)="toggleStep(step['step_id'])">
                    <span class="ptype-mini" [class]="'ptype-' + primTypeOf(step['primitive'])">{{
                      primTypeLabel(step['primitive'])
                    }}</span>
                  ...
                  @if (expandedStep() === step['step_id']) {
                    <div class="trace-step-body">
                      ...
                      @if (asArr(step['llm_calls']).length > 0) {
                        <div class="trace-section-label">
                          LLM CALLS ({{ asArr(step['llm_calls']).length }})
                        </div>
                        @for (call of asArr(step['llm_calls']); track call['seq']) {
                          <div class="llm-call">
                            <div class="llm-call-meta">
                              ...
                              <span
                                [class.ok-text]="call['parsed_ok']"
                                [class.err-text]="!call['parsed_ok']"
                              >
  ...
  styles: [
    `
      ...
      .spark-bar {
        ...
        min-height: 2px;
        transition: height var(--duration-md);
      }
      .spark-bar.conf-high {
...
export class ResultsPanelComponent {
  ...
  primTypeOf(name: string): string {
    return (name as string)?.split('.')[0] ?? '';
  }
  primTypeLabel(name: string): string {
    return this.primTypeOf(name).slice(0, 3).toUpperCase();
  }
  ...
  auditDetail(ev: any): string {
    ...
    switch (ev.type) {
      case 'plan_ready':
        return `${p['step_count']} steps · ${p['source']}`;
      case 'step_started':
        return p['primitive'] ?? '';
      case 'step_finished':
        return `${p['primitive']} · conf ${(p['confidence'] ?? 0).toFixed(2)} · ${p['citations']?.length ?? 0} cites`;
      case 'human_review_req':
        return `${p['primitive']} · conf ${(p['confidence'] ?? 0).toFixed(2)} < ${p['floor']}`;
      case 'run_finished':
        return `${p['step_count']} steps complete`;
      case 'run_error':
