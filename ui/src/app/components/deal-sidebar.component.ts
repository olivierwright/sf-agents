...
@Component({
          ...
          <div class="section-body">
            ...
            <div class="perf-section">
              <div class="perf-label-row">
                ...
                <span class="mono perf-pct" style="color: var(--color-success)">{{ perfPct(d) }}% performing</span>
              </div>
              <div class="perf-bar">
        ...
        @if (primsOpen()) {
          <div class="section-body">
            @for (p of primitives(); track p.name) {
              <div class="prim-row">
                <span class="prim-type-badge" [class]="'ptype-' + primType(p.name)">{{ primType(p.name).slice(0,3).toUpperCase() }}</span>
  ...
  styles: [`
    ...
    .section-toggle:hover { background: var(--bg-canvas); }
    .toggle-arrow { font-size: 0.625rem; color: var(--text-muted); }
    .section-label {
      font-size: 0.625rem;
      font-weight: 600;
    ...
    .section-body { padding: var(--sp-2) var(--sp-4) var(--sp-4); }
    ...
    .metric-cell { display: flex; flex-direction: column; align-items: center; gap: 1px; }
    .mc-val { font-size: 0.875rem; font-weight: 600; }
    .mc-lbl { font-size: 0.5625rem; text-transform: uppercase; letter-spacing: 0.05em; color: var(--text-muted); }

    /* Performance bar */
    .perf-section { margin-bottom: var(--sp-4); }
    .perf-label-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--sp-1); }
    .perf-label { font-size: 0.625rem; text-transform: uppercase; letter-spacing: 0.08em; color: var(--text-muted); font-weight: 600; }
    .perf-pct { font-size: 0.6875rem; }
    .perf-bar { display: flex; height: 6px; border-radius: 3px; overflow: hidden; background: var(--border); }
    .perf-fill-good { background: var(--color-success); transition: width var(--duration-slow); }
    .perf-fill-bad { background: var(--color-danger); opacity: 0.5; }

    /* EPC */
    .epc-section { margin-bottom: var(--sp-4); }
    .epc-bar { display: flex; height: 8px; border-radius: 4px; overflow: hidden; margin: var(--sp-2) 0 var(--sp-1); gap: 1px; }
    .epc-seg { min-width: 2px; transition: flex var(--duration-slow); }
    .epc-legend { display: flex; gap: var(--sp-2); flex-wrap: wrap; }
    .epc-lbl { display: flex; align-items: center; gap: 3px; font-size: 0.5625rem; color: var(--text-muted); }
    .epc-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }

    /* Documents */
    .docs-section { margin-top: var(--sp-2); }
    .doc-row {
      display: flex;
      align-items: center;
      gap: var(--sp-2);
    ...
    .doc-icon { font-size: 0.75rem; }
    .doc-name { color: var(--text-secondary); flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .doc-pages { color: var(--text-muted); font-size: 0.5625rem; flex-shrink: 0; }

    /* Use cases */
    .use-cases-list { padding: var(--sp-2) 0; }
    .uc-row {
      display: flex;
    ...
    .uc-row:hover:not([disabled]) { background: var(--bg-canvas); border-left-color: var(--accent); }
    .uc-row.active { background: rgba(27,111,107,.06); border-left-color: var(--accent); }
    .uc-row[disabled] { opacity: 0.5; cursor: not-allowed; }
    .uc-icon { font-size: 0.875rem; flex-shrink: 0; padding-top: 1px; }
    .uc-text { display: flex; flex-direction: column; gap: 2px; }
    .uc-label { font-size: 0.75rem; font-weight: 600; color: var(--text-primary); }
    .uc-q { font-size: 0.625rem; color: var(--text-muted); line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
    ...
    .ptype-connector { background: var(--color-prim); }
    .ptype-extractor { background: var(--accent); }
    .ptype-analyzer { background: var(--color-agent); }
    .ptype-validator { background: var(--color-success); }
    .prim-name { font-size: 0.625rem; color: var(--text-secondary); }
  `],
})
export class DealSidebarComponent {
  ...
  epcColor(label: string): string {
    const colors: Record<string, string> = {
      A: '#2E7D32', B: '#558B2F', C: '#F9A825', D: '#E65100', E: '#C62828', F: '#6A1A1A', G: '#4A0000',
    };
  ...
  docIcon(type: string): string {
    const icons: Record<string, string> = {
      prospectus: '📋', investor_report: '📊', spo: '🌱', impact_report: '📈',
    };
    return icons[type] ?? '📄';
  }
  ...
  catIcon(cat: string): string {
    const icons: Record<string, string> = {
      documentation: '📋', esg: '🌱', performance: '📈',
      compliance: '⚖️', structure: '🏗️', ratings: '⭐',
    };
    return icons[cat] ?? '🔍';
  }
}
